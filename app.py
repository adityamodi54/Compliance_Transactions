...
# Streamlit app title
st.title("Compliance Checker with Kafka Integration")

# File uploader widget to upload an Excel file
uploaded_file = st.file_uploader("Choose an Excel file", type="xlsx")
if uploaded_file:
    if not kafka_connected:
        st.error("Kafka connection failed: No brokers available")
    else:
        ...
        for idx, row in data.iterrows():
            ...
            # Create a transaction dictionary to send to Kafka
            transaction = row.to_dict()

            if issues:
                # If there are issues, send the transaction to the 'invalid-transactions' topic
                transaction['Issues'] = issues
                producer.send('invalid-transactions', transaction)
                ...
                # If no issues, send the transaction to the 'valid-transactions' topic
                producer.send('valid-transactions', transaction)
